<template>
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
    <MetricCard
      ...
      highlight-value
    >
      <template #description>{{ t('common.available') }}</template>
    </MetricCard>

    <MetricCard :title="t('dashboard.apiKeys')" :value="stats?.total_api_keys || 0" icon="key" tone="blue">
    ...
    <MetricCard :title="t('dashboard.todayRequests')" :value="stats?.today_requests || 0" icon="chart" tone="emerald">
      <template #description>
        {{ t('common.total') }}: {{ formatNumber(stats?.total_requests || 0) }}
      </template>
    </MetricCard>

    <MetricCard :title="t('dashboard.todayCost')" :value="`$${formatCost(stats?.today_actual_cost || 0)}`" icon="dollar" tone="violet">
      <template #description>
        <span class="text-gray-500 dark:text-gray-400">{{ t('common.total') }}: </span>
        <span class="text-violet-600 dark:text-violet-400" :title="t('dashboard.actual')">
    ...
    <MetricCard :title="t('dashboard.todayTokens')" :value="formatTokens(stats?.today_tokens || 0)" icon="cube" tone="amber">
      <template #description>
    ...
    <MetricCard :title="t('dashboard.totalTokens')" :value="formatTokens(stats?.total_tokens || 0)" icon="database" tone="blue">
      <template #description>
    ...
    <MetricCard :title="t('dashboard.performance')" :value="formatTokens(stats?.rpm || 0)" icon="bolt" tone="violet">
      <template #description>
        <span class="font-medium text-gray-700 dark:text-gray-200">RPM</span>
        <span class="mx-1 text-gray-400 dark:text-gray-500">/</span>
        <span class="font-semibold text-violet-600 dark:text-violet-400">{{ formatTokens(stats?.tpm || 0) }}</span>
        <span class="ml-1">TPM</span>
      </template>
    </MetricCard>

    <MetricCard :title="t('dashboard.avgResponse')" :value="formatDuration(stats?.average_duration_ms || 0)" icon="clock" tone="rose">
      <template #description>{{ t('dashboard.averageTime') }}</template>
    </MetricCard>
  </div>

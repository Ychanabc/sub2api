...
本项目为基于 Sub2API 的二次开发版本。在保留上游全部功能的基础上，额外新增：

- **对话审计** - 对 API 请求/响应内容及模型、账号、请求上下文进行安全审计。访问需管理员登录 + 专用二级密码 + Google Authenticator（TOTP）三重验证；每次解锁尝试都会记录 IP、浏览器指纹、浏览器、时间与结果。支持搜索、按模型筛选、导出 JSON、可配置保留天数与定时清理。
- **内容审核与风控中心** - 关键字/风险过滤链路，内置审核测试台、分组作用域与异步工作池。
- **cyber 会话屏蔽** - OpenAI cyber 策略，可对风险会话进行标记与屏蔽，屏蔽时长（TTL）可配置。
- **USDT（BEPUSDT）支付** - 在内置 EasyPay / 支付宝 / 微信 / Stripe 之外，新增 USDT 加密货币支付方式。
- **自定义品牌** - 默认站点名称为「Cats AI」。
- **界面增强** - 亮/暗主题切换、标签页导航、重构的首页，以及独立的对话审计控制台。

> 本版本持续跟随上游 [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)（当前已同步至 `0.1.151`），同时保留上述二开定制功能。

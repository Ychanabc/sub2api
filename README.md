...
This is a customized fork based on Sub2API. In addition to all upstream features, it adds:

- **Conversation Audit** - Security auditing of API request/response payloads together with model, account and request context. Access is protected by admin login + a dedicated secondary password + Google Authenticator (TOTP); every unlock attempt records IP, browser fingerprint, browser, time and result. Supports search, model filtering, JSON export, configurable retention days and scheduled cleanup.
- **Content Moderation & Risk Control** - Keyword / risk filtering pipeline with an audit test console, per-group scope and an asynchronous worker pool.
- **Cyber Session Blocking** - OpenAI "cyber" policy that can flag and block risky sessions with a configurable TTL.
- **USDT (BEPUSDT) Payment** - An additional crypto (USDT) payment provider alongside the built-in EasyPay / Alipay / WeChat Pay / Stripe.
- **Custom Branding** - Default site name is "Cats AI".
- **UI Enhancements** - Light/Dark theme toggle, tabbed navigation, a redesigned home page, and a dedicated Conversation Audit console.

> This fork tracks upstream [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api) (currently synced to `0.1.151`) while preserving the customizations above.

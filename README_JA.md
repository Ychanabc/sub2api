...
本プロジェクトは Sub2API をベースにした二次開発版です。上流のすべての機能に加えて、以下を追加しています:

- **会話監査（Conversation Audit）** - API リクエスト/レスポンスの内容を、モデル・アカウント・リクエストコンテキストとともにセキュリティ監査します。アクセスには管理者ログイン + 専用の二次パスワード + Google Authenticator（TOTP）が必要で、解除の試行ごとに IP・ブラウザフィンガープリント・ブラウザ・時刻・結果を記録します。検索、モデルによる絞り込み、JSON エクスポート、保持日数の設定、定期クリーンアップに対応。
- **コンテンツモデレーション & リスク管理** - キーワード/リスクフィルタリング、監査テストコンソール、グループ単位のスコープ、非同期ワーカープールを備えます。
- **cyber セッションブロック** - OpenAI「cyber」ポリシーにより、リスクのあるセッションをフラグ・ブロック（TTL 設定可能）。
- **USDT（BEPUSDT）決済** - 内蔵の EasyPay / Alipay / WeChat Pay / Stripe に加え、USDT 暗号資産決済に対応。
- **カスタムブランディング** - 既定のサイト名は「Cats AI」。
- **UI 強化** - ライト/ダークテーマ切替、タブナビゲーション、再設計されたホームページ、専用の会話監査コンソール。

> 本フォークは上流 [Wei-Shaw/sub2api](https://github.com/Wei-Shaw/sub2api)（現在 `0.1.151` に同期済み）を追従しつつ、上記のカスタマイズを維持します。
